...
The outcomes of the model are accessed directly via its object. We list them below:
	...
	3 - performance: A list containing the Micro-AUC from both training and testing dataset obtained at each layer. 
		- df.performance
...
train = pd.read_csv("exampleDatasetMLC/train.csv")
test = pd.read_csv("exampleDatasetMLC/test.csv")
		
train_x = train.drop([t for t in train.columns if "label"  in t],axis=1)
train_y = train[[t for t in train.columns if "label"  in t]]

test_x = test.drop([t for t in test.columns if "label"  in t],axis=1)
test_y = test[[t for t in test.columns if "label"  in t]]
